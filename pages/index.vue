	...
	<div class="flex flex-col min-h-screen transition-colors duration-300 select-none">
		<div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 border-b border-blue-100 dark:border-gray-800 py-2 sm:py-3 px-3 sm:px-4">
			...
			<AppContainer class="flex items-center justify-between gap-2 sm:gap-3">
				...
				<button class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium transition-colors whitespace-nowrap flex-shrink-0">了解更多 →</button>
			</AppContainer>
		</div>
		...
		<section class="relative min-h-[calc(100dvh-7.5rem)] md:min-h-[calc(100vh-8rem)] flex flex-col items-center justify-center bg-white dark:bg-gray-900 overflow-hidden transition-colors duration-300 py-8 md:py-0">
			...
			<div class="z-10 text-center space-y-6 md:space-y-8 px-4 animate-fade-in-up w-full max-w-4xl mx-auto">
				<h1 class="text-4xl sm:text-5xl md:text-7xl font-extrabold tracking-tight text-gray-900 dark:text-white">SCU IGDA</h1>
				<div class="space-y-2">
					<p v-if="isLoggedIn" class="text-xl sm:text-2xl md:text-3xl text-blue-600 dark:text-blue-400 font-medium">欢迎回来，同伴 {{ user?.username }}</p>
					<p class="text-xl sm:text-2xl md:text-3xl text-gray-700 dark:text-gray-300 font-light">四川大学游戏开发与研究社</p>
					<p class="text-xs sm:text-sm md:text-base text-gray-400 dark:text-gray-500 tracking-widest uppercase mt-2 md:mt-4">—— 成为游戏背后的缔造者 ——</p>
				</div>
				<div class="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 md:mt-8 justify-center">
					<BaseButton to="/activities" variant="outline" size="lg" class="min-w-[120px] sm:min-w-[140px] dark:border-gray-700 dark:text-gray-300">近期活动</BaseButton>
					<BaseButton to="/content" variant="primary" size="lg" class="min-w-[120px] sm:min-w-[140px] shadow-lg shadow-blue-200 dark:shadow-blue-900/20">内容库</BaseButton>
					<BaseButton to="/teams" variant="outline" size="lg" class="min-w-[120px] sm:min-w-[140px] dark:border-gray-700 dark:text-gray-300">组队系统
		...
		<section id="content-section" class="py-20 bg-gray-50 dark:bg-gray-800/50 transition-colors duration-300">
			<AppContainer>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
					<NuxtLink v-for="post in featuredPosts" :key="post.id" :to="`/content/${post.id}`" class="group h-full block">
						<BaseCard hover class="flex flex-col h-full cursor-pointer" padding="p-0">
							...
							<div class="p-5 flex flex-col flex-grow">
								<h3 class="text-lg font-bold text-gray-900 dark:text-white line-clamp-2 mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ post.title }}</h3>
								<p class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-4 flex-grow">{{ post.summary }}</p>
				...
				<div class="bg-gray-900 dark:bg-black rounded-2xl p-10 md:p-16 text-center text-white relative overflow-hidden border border-gray-800">
					...
					<ClientOnly>
						...
						<div v-if="!isLoggedIn" class="relative z-10 max-w-2xl mx-auto space-y-6">
							...
							<p class="text-gray-300 text-lg">无论你是程序、美术、策划还是音效师，SCU IGDA 都期待你的加入。</p>
							<div class="pt-4">
								<BaseButton to="/register" variant="primary" size="lg">申请加入社团</BaseButton>
							</div>
						</div>
						<!-- 已登录显示 -->
						<div v-else class="relative z-10 max-w-2xl mx-auto space-y-6">
							<h2 class="text-3xl md:text-4xl font-bold">准备好开始创作了吗？</h2>
							<p class="text-gray-300 text-lg">前往组队系统寻找你的伙伴，或者发布你的第一个作品。</p>
							<div class="pt-4 flex justify-center gap-4">
								<BaseButton to="/teams" variant="primary" size="lg">寻找队友</BaseButton>
								<BaseButton to="/content/create" variant="outline" size="lg" class="border-white text-white hover:bg-white hover:text-black">发布作品</BaseButton>
							</div>
						</div>
