...
<script setup lang="ts">
	// 引入 Auth 钩子 (确保你有 composables/useAuth.ts)
...
<template>
	<header
		class="sticky top-0 z-50 w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 select-none transition-colors duration-300">
		<AppContainer>
			<div class="flex items-center justify-between h-16">
				<!-- Left: Logo -->
				<div class="flex-shrink-0 flex items-center gap-3">
					<img src="~/assets/images/logo/logo-black-1.jpg" alt="SCU IGDA Logo"
				...
				<div class="flex items-center gap-4">
					...
					<ClientOnly>
						...
						<div v-if="isLoggedIn " class="hidden sm:relative sm:flex group">
							...
							<button class="flex items-center gap-2 focus:outline-none py-2">
								<span class="text-sm font-medium text-gray-700 dark:text-gray-200 max-w-[100px] truncate">{{ user?.username || '用户' }}</span>
								<img :src="user?.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=Felix'" class="w-8 h-8 rounded-full bg-gray-100 border border-gray-200 dark:border-gray-700 object-cover" alt="Avatar" />
							</button>
							<!-- 下拉菜单 (Hover显示) -->
							<div class="absolute right-0 top-full pt-2 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0">
								<div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
									<div class="py-1">
										<NuxtLink to="/dashboard" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700">个人中心</NuxtLink>
										<NuxtLink to="/content/create" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700">发布作品</NuxtLink>
										<div class="border-t border-gray-100 dark:border-gray-700 my-1"></div>
										<button @click="handleLogout" class="w-full text-left block px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">退出登录</button>
									</div>
								</div>
							</div>
						</div>
						...
						<!-- 情况B: 未登录 -->
						<NuxtLink v-else-if="!token" to="/login" class="hidden sm:inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white dark:text-black bg-gray-800 dark:bg-gray-300 rounded-md hover:bg-gray-700 dark:hover:bg-gray-200 transition-all shadow-sm">登录 / 注册</NuxtLink>
					...
					<!-- 移动端汉堡菜单按钮 -->
					<button class="md:hidden hamburger-button p-2 rounded-full text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors focus:outline-none"
						@click.stop="isMobileMenuOpen = !isMobileMenuOpen" aria-label="Toggle mobile menu">
						<!-- 汉堡图标SVG保持不变 -->
			...
			<!-- 移动端菜单 (下拉) -->
			<div v-if="isMobileMenuOpen" class="mobile-menu-container md:hidden absolute left-0 right-0 top-16 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-lg">
